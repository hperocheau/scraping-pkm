...
(async () => {
  ...
  try {
    ...
    const pageCount = pageCountMatches ? pageCountMatches[1].replace('+', '') : 'Nombre de pages non trouvé';

    // Récupérer le dernier numéro de carte trié de manière décroissante
    const { lastCardNumberDesc, productInfoList } = await processUrl(baseUrlDesc, 1, pageCount);

    // Continuez avec la deuxième URL seulement si lastCardNumberDesc est défini et s'il y a un symbole "+" dans pageCount
    if (lastCardNumberDesc !== null && pageCountText.includes('+')) {
      const { lastCardNumberAsc, updatedProductInfoList } = await processUrl(baseUrlAsc, 1, lastCardNumberDesc, pageCount);

      // Vérifier si lastCardNumberAsc est défini pour éviter l'écriture du fichier si le traitement est arrêté
      if (lastCardNumberAsc !== null) {
        // Écrire les informations dans le fichier JSON
        const fileName = 'test.json';
        await fs.writeFile(fileName, JSON.stringify(updatedProductInfoList, null, 2), 'utf-8');
  ...
  async function processUrl(url, startPage, lastCardNumberDesc = null, pageCount = null) {
    ...
    for (let currentPage = startPage; currentPage <= (pageCount || 15); currentPage++) {
      ...
      // Si lastCardNumberDesc est fourni et la dernière carte triée de manière ascendante est supérieure ou égale
      // à la dernière carte triée de manière descendante, arrêtez le traitement
      if (lastCardNumberDesc && currentPageProductInfoList.length > 0 && parseInt(currentPageProductInfoList[currentPageProductInfoList.length - 1].cardNumber, 10) >= parseInt(lastCardNumberDesc, 10)) {
        console.log('Arrêt du traitement car la dernière carte triée de manière ascendante est supérieure ou égale à la dernière carte triée de manière descendante.');
        break;
      }
    ...
    return {
      lastCardNumber: productInfoList.length > 0 ? productInfoList[productInfoList.length - 1].cardNumber : null,
      updatedProductInfoList: productInfoList,
    };
  }
})();
